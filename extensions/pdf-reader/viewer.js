...
const system = speechAdapter.create();

// Kokoro-82M, running on this machine from files inside the extension folder.
// It reports word timing for every voice, which is what chrome.tts only manages
// on some machines. speech/dual puts both behind one interface and falls back to
// chrome.tts if the model was never fetched or will not start.
const kokoro = kokoroAdapter.create({
  HeadTTS,
  onstatus: (text) => text && console.log(`[pdf-reader] ${text}`),
});

const speech = dualFactory.create({
  kokoro,
  system,
  // The model is 326MB and fetch-assets.sh may never have been run. Say so once,
  // in the notice panel, and carry on with chrome.tts.
  onfallback: (message) => {
    console.error(`[pdf-reader] neural voice unavailable: ${message}`);
