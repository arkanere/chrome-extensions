...
// LOCAL CHANGE (chrome-extensions). connect() is async and is called from the
// message handler without a catch, so anything that throws in it — a missing
// file, a backend that will not start — becomes an unhandled rejection and this
// worker simply goes quiet. HeadTTS then waits for a `ready` that never comes
// and reports, at best, a timeout. Measured: a failed tokenizer load hung for
// over three minutes with nothing in the console. This makes the failure speak.
self.addEventListener("unhandledrejection", (event) => {
  const reason = event.reason;
  console.error("[headtts worker] unhandled rejection:", reason);
  event.preventDefault();
  // Rethrown from a task so it is an *uncaught exception* rather than a
  // rejection. That distinction is the whole point: HeadTTS rejects its connect
  // promise from worker.onerror, which fires for exceptions and never for
  // rejections. A posted message would not do — during connect its onmessage
  // handler reads only "progress" and "ready" and drops everything else.
  setTimeout(() => {
...
async function connect() {
  ...
  try {
    ...
    // LOCAL CHANGE (chrome-extensions): keep the cause. See the catch below.
    throw new Error(`Importing modules failed: ${(error && (error.message || error)) || "no reason given"}`);
  }
  ...
  try {
    ...
    // LOCAL CHANGE (chrome-extensions): keep the cause, and say which device and
    // dtype were being loaded. Upstream throws four bare words; by the time they
    // have passed up through headtts.mjs and speech/dual that is all anyone gets
    // — a message naming no file, no backend and no reason. Three layers of this
    // repo's debugging went into rediscovering what this line already knew.
    throw new Error(
      `Loading models failed (${settings.device}/${settings.dtype}): ` +
      `${(error && (error.message || error)) || "no reason given"}`
    );
  }
