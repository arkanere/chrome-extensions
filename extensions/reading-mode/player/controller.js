...
//
// It knows nothing about the EPUB container, the DOM or any particular voice. It
// sees the book only through core/document-model and the engine only through the
// section 5.1 adapter.
//
// Copied from pdf-reader, with pages renamed to sections. The one real difference
// is what prefetch costs: parsing a section here means rendering a whole chapter
// (section 6), not just pulling a page's text items.
...
export function create(model, speech, config = {}) {
  ...
  // Sentences arrive as sections are parsed, so walk the model forward until the
  // id exists or the book runs out.
  async function ensureSentence(id) {
    while (id >= model.sentences.length && model.parsedSections < model.sectionCount) {
      await model.ensureSections(model.parsedSections + 1);
    }
    return id < model.sentences.length;
  }

  // Rendering and walking a chapter mid-sentence would be heard as a gap, so pull
  // the next one in while the current sentence is still being spoken. Audio
  // pre-buffering is a separate, deferred idea (section 6); this is just the text.
  //
  // `section` is an index; ensureSections takes a count, so parsing section n
  // means asking for n + 1.
  function prefetch(section) {
    if (section < model.sectionCount) model.ensureSections(section + 1).catch(() => {});
  }
  ...
  return {
    ...
    // stop-and-remember path as pause. Phase 7's controls use these.
    setVoice(id) {
      voice = id;
      if (playing) {
        speech.stop();
